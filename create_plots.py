...
plt.ylabel('$N$')
cbar = plt.colorbar(c)
cbar.set_label('Binding energy/ nucleon (MeV)')
plt.title('Binding Energy/nucleon of all nuclei')
plt.savefig('Binding Energy 2D projection.png', bbox_inches='tight')

plt.clf()
plt.scatter(Zs+Ns,Es)
plt.xlabel('$A$')
plt.ylabel('Binding energy/ nucleon (MeV)')
plt.title('Binding Energy/nucleon of the most stable nuclei')
plt.savefig('Binding Energy vs Mass Number.png', bbox_inches='tight')


fig = go.Figure(data=[go.Surface(z=E.T, x=ZZ, y=NN, colorscale='RdBu',
								colorbar={'title':'Binding Energy/ nucleon (MeV)'},
